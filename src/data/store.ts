...
export class Store {
  ...
  getLastTweetId(userId: string): string | undefined {
    const row = this.db.prepare(`SELECT last_tweet_id FROM user_latest WHERE user_id = ?`).get(userId) as { last_tweet_id?: string } | undefined;
  ...
  queryTweets(opts: {
    ...
    const rows = this.db.prepare(sql).all(...params) as (TweetEntity & { username: string })[];
    return rows;
  }
