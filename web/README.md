...
│   ├── tweets/
│       ├── TweetCard.tsx      推文卡片（头像/引用卡片/媒体预览）
│       ├── FilterBar.tsx      筛选栏（搜索 / 用户 / 日期范围）
│       └── MediaPreviewModal.tsx  媒体大图预览弹窗
│   └── ui/
│       └── UserAvatar.tsx     统一头像组件（优先真实头像）
├── lib/
...
- 点击任意推文右上角时间，可统一切换全部动态卡片的时间格式（相对时间 / 详细时间）

### 推文管理 `/tweets`
- 全量推文列表，每页 30 条分页
- 筛选：用户下拉、关键词搜索、起止日期选择、是否包含历史已移除用户
- 页面结构分层为：筛选区、推文列表区、分页区
- 推文卡片展示头像（优先真实头像）、用户名、发布时间（固定详细时间）、正文、语言标签
- 推文卡片展示采集状态标签（监控中采集 / 作者已移除等）与重点用户标识
- 点击媒体先弹窗预览大图，在弹窗中按需跳转原文链接

### 用户管理 `/users`
- 监控用户列表优先展示，支持按“全部/监控中/未监控/重点”筛选及关键词搜索
- 单行信息分为身份、统计、操作三区，显示真实头像、推文计数、最近活跃时间、监控状态
- **静态模式**：支持添加用户、停止监控、恢复监控（实时写入 config.json）
- 新增用户后会尝试即时拉取头像并落库
- 停止监控不会删除历史推文，仅从当前抓取目标中移除
- **动态模式**：只读，提示由关注列表自动同步
- 行内图标按钮支持操作：重点切换、停止/恢复监控、跳转 X 主页（含 tooltip）

### 数据分析 `/analytics`
- **每日推文趋势**：柱状图，支持切换 7 / 14 / 30 / 90 天范围
- **用户发帖排行**：带头像的横向进度条排行榜，前 10 名
